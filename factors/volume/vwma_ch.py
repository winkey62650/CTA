...
def signal(df, para=[30], proportion=1, leverage_rate=1):
    ...
    condition2 = df['volume'] > df['vol_ma'] * 1.5
    df.loc[condition1 & condition2, 'signal_long'] = 1

    # 做多平仓信号: 价格回落VWAP或成交量萎缩
    condition1 = df['close'] < df['vwap']
    condition2 = df['volume'] < df['vol_ma'].shift(1)
    ...
    # 删除中间变量
    df.drop(['vwap', 'price_high', 'price_low', 'vol_ma', 'signal_long', 'signal_short'], axis=1, inplace=True)
